...
class NeuralNetworkService(IScoped):
    @inject
    def __init__(self,
    ...
    def backup_models(self):
        ...
        try:
            ...
            self.logger.error("(backupModels) Unable to copy file: {0}".format(str(e)))
        except Exception as ex:
            self.logger.error("(backupModels) Unexpected error: {0}".format(sys.exc_info()))
    ...
    def run_training_operation(self):
        try:
            ...
            self.logger.info("Starting Neural Network algorithm...")
            
            path = os.path.join(self.other_config.root_directory, self.other_config.model_path, 'test.txt')
            file1 = open(path, 'a')
            file1.writelines("Writing this file for testing")
            file1.close()

            self.logger.info("Reading DB to get all data...")
            ...
            self.logger.info("Datas are fetched.")

            ###########################################################################################################
            # Pre-Processing
            self.logger.info("Pre-processing is started.")
            ...
            y = to_categorical(y)
            self.logger.info("Pre-processing is done.")

            ###########################################################################################################
            # Training DataSet
            self.logger.info("ANN Model is training....")
            ...
            self.logger.info("ANN Model is finished....")

            ###########################################################################################################
            # Test DataSet
            loss, accuracy = model.evaluate(test_set_x, test_set_y)
            self.logger.info("!!!TEST RESULTS!!! loss:{0}, accuracy:{1}".format(loss, accuracy))

            ###########################################################################################################
            # Backup legacy models to specified file.
            self.backup_models()
            self.logger.info("Model is backuped.")

            # Model check and finiliaze.
            if accuracy > self.ann_config.minimum_accuracy:
                self.visualization(history)

                self.logger.info("Accuracy is enough to replace the model. :)")
                ...
                output.close()
                self.logger.info("One-Hot-Encoding is saved...")

            else:
                self.logger.info("Accuracy is not enough to replace the model. :(")

        except Exception as e:
            self.logger.error("neurol network main error: {}".format(str(e)))
        finally:
            
            later = time.time()
            difference = int(later - now)
            self.logger.info(f"Operation finishded in {difference} seconds")
